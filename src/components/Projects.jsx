...
const Projects = () => {
  ...
  const filtered = projects.filter((p) => {
    const matchCategory = activeFilter === "all" || p.category === activeFilter || p.techStack.includes(activeFilter);
              ...
              >
                <div className="relative h-48 overflow-hidden">
                  <div className="absolute inset-0 bg-gradient-to-br from-primary/20 via-secondary/20 to-accent/20 flex items-center justify-center">
                    <span className="text-4xl font-bold gradient-text opacity-50">{project.title[0]}</span>
                  </div>
                ...
                <div className="p-5">
                  ...
                  <div className="flex flex-wrap gap-2 mb-3">
                    ...
                    {project.techStack.length > 4 && (
                      <span className="px-2 py-0.5 text-gray-400 rounded text-xs">+{project.techStack.length - 4}</span>
                    )}
                  </div>
          ...
          >
            ...
            >
              <div className="flex justify-between items-start mb-4">
                ...
                <button onClick={() => setSelected(null)} className="p-1 hover:bg-white/10 rounded-lg transition-all cursor-pointer">
                  <IoClose size={24} />
                </button>
              </div>
              ...
              <div className="h-48 rounded-xl bg-gradient-to-br from-primary/20 via-secondary/20 to-accent/20 flex items-center justify-center mb-6">
                <span className="text-6xl font-bold gradient-text opacity-30">{selected.title[0]}</span>
              </div>

              <p className="text-gray-400 mb-4">{selected.longDescription}</p>
