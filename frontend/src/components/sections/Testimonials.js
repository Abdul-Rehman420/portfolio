    ...
    <section id="testimonials" className="relative py-20">
      <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-20">
        ...
        <motion.p initial={{ opacity: 0 }} whileInView={{ opacity: 1 }} viewport={{ once: true }}
          className="text-center text-muted mb-12 max-w-2xl mx-auto">What people say about working with me</motion.p>
            ...
            <AnimatePresence mode="wait" custom={dir}>
              <motion.div key={current} custom={dir} variants={variants} initial="enter" animate="center" exit="exit" transition={{ duration: 0.3 }}
                ...
                <div className="w-20 h-20 rounded-full bg-linear-to-br from-primary via-secondary to-accent p-0.5 mx-auto mb-4">
                  <div className="w-full h-full rounded-full bg-body flex items-center justify-center">
                ...
                <p className="text-muted leading-relaxed italic">&ldquo;{t.review}&rdquo;</p>
              </motion.div>
            </AnimatePresence>
          </div>
            ...
            <div className="flex items-center gap-2">
              {testimonials.map((_, i) => (
                <button key={i} onClick={() => { setDir(i > current ? 1 : -1); setCurrent(i); }}
                  className={`w-2 h-2 rounded-full transition-all cursor-pointer ${i === current ? 'bg-primary w-6' : 'dot-inactive'}`} />
              ))}
            </div>
